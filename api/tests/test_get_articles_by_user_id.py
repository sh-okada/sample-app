...
@pytest.mark.parametrize(
    ...
    [
        pytest.param(
            "caa93979-2256-42f0-8e83-55144674613b",
            200,
            {
                "values": [
                    {
...
def test_レスポンス(id: str, status_code: int, json_response: dict):
    ...
    with freeze_time(datetime(2025, 7, 23, 0, 0, 0)):
        response = client.get(f"/api/users/{id}/articles")
...
@pytest.mark.parametrize(
    "id, query_params, json_response",
    [
        pytest.param(
            "6e2aa5a1-f792-47b8-9393-58fd657e7451",
            {},
            {
                "values": [
                    {
...
def test_クエリパラメータ(id: str, query_params: dict, json_response: list[dict]):
    users = [
        db_models.User(
    ...
    with freeze_time(datetime(2025, 7, 23, 0, 0, 0)):
        response = client.get(f"/api/users/{id}/articles", params=query_params)
